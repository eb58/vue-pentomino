...
const shuffle = (xs) =>
   xs.reduce(
      (xs, x, i) => (
         feedX(Math.floor(Math.random() * xs.length), (j) => ([xs[i], xs[j]] = [xs[j], xs[i]])), xs
      ),
      xs
   )
...
const matrix = {
   ...
   redim: (mat, nrows, ncols, defVal = 0) =>
      range(nrows).map((r) => range(ncols).map((c) => mat[r]?.[c] || defVal)),
   ...
   translate: (mat, dr, dc, defVal = 0) =>
      range(mat.length).map((r) =>
         range(mat[0].length).map((c) => mat[r - dr]?.[c - dc] || defVal)
      ),
